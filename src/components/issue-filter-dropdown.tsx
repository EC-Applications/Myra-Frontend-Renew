        ...
        {category.options?.map((option) => {
          ...
          return (
            <button
              ...
              className={`w-full px-4 py-2.5 flex items-center gap-3 text-sm text-left transition-all duration-150 border-b border-zinc-800/50 last:border-b-0 ${checked
            ...
            >
              ...
              <div className="flex-1 flex items-center justify-between">
                <span
                  className={`font-normal ${checked ? "text-[#838ef1]" : "text-white"
                    }`}
                >
                  {option.label}
                </span>
...
export function IssueFilterDropdown({ onFilterChange }: FilterDropdownProps = {}) {
  ...
  const label = useSelector((state: RootState) => state.issuesLabel);
  ...
  const statusFromRedux = useSelector((state: RootState) => state.issuesStatus);
  ...
  const renderFilterPopover = () => (
    <Popover>
      <PopoverTrigger asChild>
        <button className="inline-flex items-center justify-center gap-2 px-3 py-2 text-sm font-medium border-zinc-700 rounded-md hover:bg-[#e8e8e8] dark:hover:bg-zinc-900 transition-colors dark:text-zinc-300 relative">
