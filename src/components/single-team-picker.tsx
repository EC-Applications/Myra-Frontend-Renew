...
export const SingleTeamPicker = ({
  ...
  return (
    <DropdownMenu open={open && teams.length > 1} onOpenChange={setOpen}>
              ...
              <IconPicker
                ...
                value={
                  typeof value?.icon === "object"
                    ? {
                        ...value.icon,
                        icon: parseEmojiFromUnicode(value.icon.icon), 
                      }
                    : value?.icon
                      ? {
      ...
      <DropdownMenuContent
        align="start"
        className="w-[260px] p-0 dark:bg-[#1c1d1f]"
      >
        <Input
        ...
        <div className="max-h-[280px] px-1 py-1 overflow-y-auto ">
          {filteredTeams.map((team) => (
            <DropdownMenuItem
              key={team.id}
              className="group relative flex h-8 cursor-pointer items-center rounded-sm px-2 py-5 text-[13px] transition-colors dark:hover:bg-[#292b30] border-0 font-semibold pointer"
              ...
              <IconPicker
                ...
                value={
                  typeof team?.icon === "object"
                    ? {
                        ...team.icon,
                        icon: parseEmojiFromUnicode(team.icon.icon), 
                      }
                    : team?.icon
                      ? {
